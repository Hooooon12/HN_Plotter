...
void makePlots_ZZ_mumu_HighPt(){

  string histline;
  ifstream in("histList_ZZ_mumu_HighPt.txt");
  // Line loop
  while(getline(in, histline)){
    ...
    PDname = "SingleMuon";
    txt_region = region(0,2)+"_"+region(3,7);
    ...
    for(int it_y=0; it_y<year.size(); it_y++){
      ...
      f_MC[0][it_y]  = new TFile(workdir+file_path+analyzer+"_"+skim+"_ZZTo4L_powheg.root");
      //f_MC[14][it_y]  = new TFile(workdir+file_path+analyzer+"_"+skim+"_"+ZGname.at(it_y)+".root");
      ...
      // MC : VVV
      f_MC[1][it_y]  = new TFile(workdir+file_path+analyzer+"_WWW.root");
      f_MC[2][it_y]  = new TFile(workdir+file_path+analyzer+"_WWZ.root");
      f_MC[3][it_y]  = new TFile(workdir+file_path+analyzer+"_WZZ.root");
      f_MC[4][it_y]  = new TFile(workdir+file_path+analyzer+"_ZZZ.root"); //JH
      // MC : Top
      f_MC[5][it_y]  = new TFile(workdir+file_path+analyzer+"_"+skim+"_ttWToLNu.root");
      f_MC[6][it_y] = new TFile(workdir+file_path+analyzer+"_"+skim+"_ttZToLLNuNu.root");
      f_MC[7][it_y] = new TFile(workdir+file_path+analyzer+"_"+skim+"_ttHToNonbb.root");
      ...
      // MC : Higgs
      //f_MC[8][it_y] = new TFile(workdir+file_path+analyzer+"_ggHToZZTo4L.root");
      //f_MC[9][it_y] = new TFile(workdir+file_path+analyzer+"_VBF_HToZZTo4L.root");
      //f_MC[18][it_y] = new TFile(workdir+file_path+analyzer+"_VHToNonbb.root");
      // MC : ggZZTo4L
      f_MC[8][it_y] = new TFile(workdir+file_path+analyzer+"_ggZZTo2e2mu.root");
      f_MC[9][it_y] = new TFile(workdir+file_path+analyzer+"_ggZZTo2e2tau.root");
      f_MC[10][it_y] = new TFile(workdir+file_path+analyzer+"_ggZZTo2mu2tau.root");
      f_MC[11][it_y] = new TFile(workdir+file_path+analyzer+"_ggZZTo4e.root");
      f_MC[12][it_y] = new TFile(workdir+file_path+analyzer+"_ggZZTo4mu.root");
      f_MC[13][it_y] = new TFile(workdir+file_path+analyzer+"_ggZZTo4tau.root");
      ...
      //h_MC[0][it_y]->SetFillColor(kRed);
      //hs->Add(h_MC[0][it_y]);
      h_Bundle[2][it_y]->SetLineWidth(0);
      h_Bundle[2][it_y]->SetFillColor(kRed);
      ...
      if(h_Bundle[2][it_y]) h_Error[it_y]->Add(h_Bundle[2][it_y]); //JH
      ...
      //lg->AddEntry(h_MC[1][it_y], "t#bar{t}", "f"); //JH
      //lg->AddEntry(h_MC[0][it_y], "ZZ", "f");
      //lg->AddEntry(h_MC[1][it_y], "WZ", "f");
      lg->AddEntry(h_Bundle[0][it_y], "ttV", "f");
      lg->AddEntry(h_Bundle[1][it_y], "VVV", "f");
      lg->AddEntry(h_Bundle[2][it_y], "ZZ", "f"); //JH
      lg->SetBorderSize(0);
      lg->SetTextSize(0.03);
