...
void makePlots_WZ_mumu_POGTight(){

  string histline;
  ifstream in("histList_WZ_mumu_POGTight.txt");
  // Line loop
  while(getline(in, histline)){
    ...
    for(int it_y=0; it_y<year.size(); it_y++){
      ...
      //f_MC[0][it_y]  = new TFile(workdir+file_path+analyzer+"_"+skim+"_ZZ_pythia.root");
      //f_MC[1][it_y]  = new TFile(workdir+file_path+analyzer+"_"+skim+"_WZ_pythia.root"); //JH
      ...
      // MC : VVV
      f_MC[2][it_y]  = new TFile(workdir+file_path+analyzer+"_WWW.root");
      ...
      // MC : Top
      f_MC[6][it_y]  = new TFile(workdir+file_path+analyzer+"_"+skim+"_ttWToLNu.root");
      f_MC[7][it_y] = new TFile(workdir+file_path+analyzer+"_"+skim+"_ttZToLLNuNu.root");
      f_MC[8][it_y] = new TFile(workdir+file_path+analyzer+"_"+skim+"_ttHToNonbb.root");
