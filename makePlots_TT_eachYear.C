...
void makePlots_TT_eachYear(){

  string histline;
  ifstream in("histList_TT.txt");
  // Line loop
  while(getline(in, histline)){
    ...
    if(region == "TTmm"){
      txt_region = "t#bar{t} #mu^{#pm}#mu^{#mp}";
      PDname = "DoubleMuon";
    }
    if(region == "TTee"){
      txt_region = "t#bar{t} e^{#pm}e^{#mp}";
      PDname = "DoubleEG";
    }
    if(region == "TTemu"){
      txt_region = "t#bar{t} e^{#pm}#mu^{#mp}";
      PDname = "MuonEG";
    }
