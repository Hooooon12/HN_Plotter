...
void makePlots_WZ_HighPt(){

  string histline;
  ifstream in("histList_WZ_HighPt.txt");
  // Line loop
  while(getline(in, histline)){
    ...
    for(int it_y=0; it_y<year.size(); it_y++){
      ...
      f_MC[1][it_y]  = new TFile(workdir+file_path+analyzer+"_"+skim+"_WZTo3LNu_powheg.root"); //JH
      ...
      for(int it_mc=8; it_mc<MCNumber; it_mc++){
